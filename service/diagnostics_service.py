...
from domain.exceptions import ConfigurationError, ValidationError, ExternalServiceError, DataParsingError
...
class DiagnosticsService:
    ...
    def __init__(self):
        ...
        if not self.api_key:
            logger.critical("Failed to initialize DiagnosticsService: GOOGLE_API_KEY environment variable is not set.")
    ...
    def _validate_telemetry(self, input_data: TelemetryInput) -> None:
        ...
        if not input_data.cpu.strip() or not input_data.gpu.strip() or not input_data.application.strip() or not input_data.os_name or not input_data.storage:
            logger.warning(f"Telemetry validation failed. Input: {input_data}")
            raise ValidationError("Missing arguments. Please fill in all required telemetry fields.")
    ...
    def run_diagnostics(self, telemetry: TelemetryInput) -> DiagnosticResponse:
        ...
        try:
            ...
            raise DataParsingError(f"Failed to hydrate domain models from payload: {exc}") from exc
